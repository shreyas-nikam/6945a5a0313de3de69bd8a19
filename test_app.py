...
# To ensure AppTest.from_file works, save the provided Streamlit app code
# into a file named 'app.py' in the same directory where these tests are run.
# For the purpose of this response, we assume 'app.py' exists with the given content.

# Example of how you might create app.py dynamically if needed for a self-contained test file:
# app_code = """
...
def test_initial_page_and_navigation():
    """Test initial page content and sidebar navigation."""
    at = AppTest.from_file("app.py").run()

    # Test Introduction page content
    assert at.markdown[0].value == "# Automated Financial Report Data Extraction with SmolDocling"
    assert "SmolDocling" in at.markdown[2].value
    assert "bounding box coordinates" in at.markdown[4].value

    # Navigate to "Upload Document"
    at.sidebar.radio[0].set_value("Upload Document").run()
    assert at.markdown[0].value == "## 2. Upload and Process Financial Report"
    assert at.info[0].value == "Upload a PDF or use the sample generation tool to simulate the workflow."

    # Navigate to "Review & Export" (initially expecting a warning)
    at.sidebar.radio[0].set_value("Review & Export").run()
    assert at.markdown[0].value == "## 3. Review & Export Data"
    assert at.warning[0].value == "No data processed yet. Please go to 'Upload Document' and process a file first."

def test_upload_document_page_sample_report_and_processing():
    """Test loading sample report, processing it, and checking results on Upload Document page."""
    at = AppTest.from_file("app.py").run()
    at.sidebar.radio[0].set_value("Upload Document").run()

    # Click "Use Sample Financial Report"
    at.button[0].click().run() 

    # Verify sample loaded
    assert at.session_state["uploaded_pdf_file"] is not None
    assert at.success[0].value == "Sample PDF loaded!"

    # Click "Process Document"
    at.button[1].click().run()

    # Verify processing complete message
    assert at.success[1].value == "Processing Complete! Go to 'Review & Export' or scroll down."

    # Verify session state variables are populated
    assert at.session_state["processed_image"] is not None
    assert "<root>" in at.session_state["doctags_xml"]
    assert at.session_state["parsed_document"] is not None
    assert at.session_state["financial_metrics"] is not None
    assert at.session_state["visualization_elements"] is not None
    assert at.session_state["extracted_df_csv_content"] is not None
    assert at.session_state["extracted_json_content"] is not None

    # Verify content displayed on the page
    assert at.image[0].caption == "Document Analysis Overlay"
    assert "FinTech Corp Financial Report 2023" in at.code[0].value
    
    # Check extracted tables display
    assert at.dataframe[0].value.iloc[0, 0] == "Total Revenue"
    assert at.dataframe[0].value.iloc[0, 1] == "$1234.56"
    assert at.dataframe[0].value.columns.tolist() == ['Metric', '2023 (in Millions)'] # Check headers

    # Verify extracted financial metrics
    expected_metrics = {
        "Total Revenue": "$1234.56",
        "Operating Expenses": "$850.00",
        "Net Income": "$384.56",
        "Earnings Per Share": "0.52"
    }
    assert at.session_state["financial_metrics"] == expected_metrics
    
def test_review_export_page_with_processed_data():
    """Test the 'Review & Export' page displays correct data after processing."""
    at = AppTest.from_file("app.py").run()
    
    # First, navigate to Upload Document and process a file to populate session state
    at.sidebar.radio[0].set_value("Upload Document").run()
    at.button[0].click().run()  # Use Sample Financial Report
    at.button[1].click().run()  # Process Document

    # Now navigate to "Review & Export"
    at.sidebar.radio[0].set_value("Review & Export").run()

    # Verify no warning is displayed (as data is processed)
    assert not at.warning

    # Verify Key Financial Metrics dataframe
    assert at.dataframe[0].value.columns.tolist() == ['Metric', 'Value']
    assert at.dataframe[0].value.iloc[0, 0] == "Total Revenue"
    assert at.dataframe[0].value.iloc[0, 1] == "$1234.56"

    # Verify Extracted Tables dataframe
    assert at.dataframe[1].value.columns.tolist() == ['Metric', '2023 (in Millions)', '2022 (in Millions)']
    assert at.dataframe[1].value.iloc[0, 0] == "Total Revenue"
    assert at.dataframe[1].value.iloc[0, 1] == "$1234.56"

    # Verify download buttons have data
    assert at.download_button[0].label == "Download Data as CSV"
    assert at.download_button[0].data is not None
    assert "Total Revenue,$1234.56" in at.download_button[0].data

    assert at.download_button[1].label == "Download Data as JSON"
    assert at.download_button[1].data is not None
    assert '"Total Revenue": "$1234.56"' in at.download_button[1].data

# You can run these tests using pytest:
# 1. Save the Streamlit app code as `app.py`.
# 2. Save the above test code as `test_app.py` (or similar).
